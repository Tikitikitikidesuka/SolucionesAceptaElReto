...
inline c_int getPeriodo(c_int pos, c_int size, c_int desplazamientos[MAX_SIZE]) {
    c_int periodo = 1;
...
int main() {
    ...
    while(longitud != 0) {
        ...
        c_int lcmResult = getPeriodo(0, longitud, desplazamientos);
        for(c_int it = 1; it < longitud; ++it)
            lcmResult = lcm(lcmResult, getPeriodo(it, longitud, desplazamientos));
